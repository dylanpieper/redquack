...
optimize_data_types <- function(conn, data_table_ref, log_table_ref) {
  if (!is_db_class(conn)) {
    log_message(log_table_ref, "WARNING", "Connection is not DuckDB, skipping optimization")
    return(invisible(NULL))
  }

  log_message(log_table_ref, "INFO", "Optimizing column data types for DuckDB")
  ...
  log_message(log_table_ref, "INFO", paste("Found", nrow(column_info), "columns to check"))
    ...
    tryCatch(
      {
        ...
        if (integer_check) {
          ...
          log_message(log_table_ref, "INFO", paste("Column", col, "converted to INTEGER"))
          next
        }
        ...
        if (numeric_check) {
          ...
          log_message(log_table_ref, "INFO", paste("Column", col, "converted to DOUBLE"))
          next
        }
        ...
        if (date_check) {
          ...
          log_message(log_table_ref, "INFO", paste("Column", col, "converted to DATE"))
          next
        }
        ...
        if (timestamp_check) {
          ...
          log_message(log_table_ref, "INFO", paste("Column", col, "converted to TIMESTAMP"))
        }
      },
      error = function(e) {
        log_message(log_table_ref, "WARNING", paste("Unable to optimize column", col, ":", e$message))
      }
    )
  }
  ...
  tryCatch(
    {
      ...
      log_message(log_table_ref, "INFO", "Enabled automatic compression for DuckDB")
    },
    error = function(e) {
      log_message(log_table_ref, "WARNING", paste("Unable to enable compression:", e$message))
    }
  )

  log_message(log_table_ref, "INFO", "Column optimization completed")
  return(invisible(NULL))
}
